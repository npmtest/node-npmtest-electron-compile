...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-electron-compile/node_modules/electron-compile/lib/compile-cache.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header low">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-electron-compile/node_modules/electron-compile/lib/compile-cache.js</span></h1>
    <h2>
        
        Statements: <span class="metric">9.73% <small>(18 / 185)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="path"><a href="../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-electron-compile/node_modules/electron-compile/lib/</a> &#187; compile-cache.js</div>
</div>
<div class="body">
...
class CompileCache {
   ...
   */
  constructor<span class="fstat-no" title="function not covered" >(cachePath, fileChangeCache) {</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    this.cachePath = cachePath;</span>
<span class="cstat-no" title="statement not covered" >    this.fileChangeCache = fileChangeCache;</span>
<span class="cstat-no" title="statement not covered" >    this.sourceMapPath = sourceMapPath || this.cachePath;</span>
  }
&nbsp;
  /**
   ...
   */
  static createFromCompiler<span class="fstat-no" title="function not covered" >(cachePath, compiler, fileChangeCache) {</span>
<span class="cstat-no" title="statement not covered" >    let readOnlyMode = arguments.length &gt; 3 &amp;&amp; arguments[3] !== undefined ? arguments[3] : false;</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    let newCachePath = null;</span>
<span class="cstat-no" title="statement not covered" >    let getCachePath = () =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      if (newCachePath) <span class="cstat-no" title="statement not covered" >return newCachePath;</span></span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      const digestObj = {</span>
        name: compiler.name || Object.getPrototypeOf(compiler).constructor.name,
        version: compiler.getCompilerVersion(),
        options: compiler.compilerOptions
      };
&nbsp;
<span class="cstat-no" title="statement not covered" >      newCachePath = _path2.default.join(cachePath, (0, _digestForObject2.default)(digestObj));</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      d(`Path for ${digestObj.name}: ${newCachePath}`);</span>
<span class="cstat-no" title="statement not covered" >      d(`Set up with parameters: ${JSON.stringify(digestObj)}`);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (!readOnlyMode) <span class="cstat-no" title="statement not covered" >_mkdirp2.default.sync(newCachePath);</span></span>
<span class="cstat-no" title="statement not covered" >      return newCachePath;</span>
    };
&nbsp;
<span class="cstat-no" title="statement not covered" >    let ret = new CompileCache('', fileChangeCache);</span>
<span class="cstat-no" title="statement not covered" >    ret.getCachePath = getCachePath;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    const newSourceMapPath = sourceMapPath;</span>
<span class="cstat-no" title="statement not covered" >    ret.getSourceMapPath = () =&gt; <span class="cstat-no" title="statement not covered" >newSourceMapPath || getCachePath();</span></span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      d(`Fetching ${filePath} from cache`);</span>
<span class="cstat-no" title="statement not covered" >      let hashInfo = yield _this.fileChangeCache.getHashForPath(_path2.default.resolve(filePath));</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      let code = null;</span>
<span class="cstat-no" title="statement not covered" >      let mimeType = null;</span>
<span class="cstat-no" title="statement not covered" >      let binaryData = null;</span>
<span class="cstat-no" title="statement not covered" >      let dependentFiles = null;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      let cacheFile = null;</span>
<span class="cstat-no" title="statement not covered" >      try {</span>
<span class="cstat-no" title="statement not covered" >        cacheFile = _path2.default.join(_this.getCachePath(), hashInfo.hash);</span>
...
<span class="cstat-no" title="statement not covered" >          d("File is binary, reading out info");</span>
<span class="cstat-no" title="statement not covered" >          let info = JSON.parse((yield _promise.pfs.readFile(cacheFile + '.info')));</span>
<span class="cstat-no" title="statement not covered" >          mimeType = info.mimeType;</span>
<span class="cstat-no" title="statement not covered" >          dependentFiles = info.dependentFiles;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          binaryData = hashInfo.binaryData;</span>
<span class="cstat-no" title="statement not covered" >          if (!binaryData) {</span>
<span class="cstat-no" title="statement not covered" >            binaryData = yield _promise.pfs.readFile(cacheFile);</span>
<span class="cstat-no" title="statement not covered" >            binaryData = yield _promise.pzlib.gunzip(binaryData);</span>
          }
        } else {
<span class="cstat-no" title="statement not covered" >          let buf = yield _promise.pfs.readFile(cacheFile);</span>
<span class="cstat-no" title="statement not covered" >          let str = (yield _promise.pzlib.gunzip(buf)).toString('utf8');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >          result = JSON.parse(str);</span>
<span class="cstat-no" title="statement not covered" >          code = result.code;</span>
<span class="cstat-no" title="statement not covered" >          mimeType = result.mimeType;</span>
<span class="cstat-no" title="statement not covered" >          dependentFiles = result.dependentFiles;</span>
        }
      } catch (e) {
<span class="cstat-no" title="statement not covered" >        d(`Failed to read cache for ${filePath}, looked in ${cacheFile}: ${e.message}`);</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      return { hashInfo, code, mimeType, binaryData, dependentFiles };</span>
    })();
  }
&nbsp;
  /**
   ...
   */
  save<span class="fstat-no" title="function not covered" >(hashInfo, codeOrBinaryData, mimeType, dependentFiles) {</span>
<span class="cstat-no" title="statement not covered" >    var _this2 = this;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return _asyncToGenerator(<span class="fstat-no" title="function not covered" >function* () {</span></span>
<span class="cstat-no" title="statement not covered" >      let buf = null;</span>
<span class="cstat-no" title="statement not covered" >      let target = _path2.default.join(_this2.getCachePath(), hashInfo.hash);</span>
<span class="cstat-no" title="statement not covered" >      d(`Saving to ${target}`);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (hashInfo.isFileBinary) {</span>
<span class="cstat-no" title="statement not covered" >        buf = yield _promise.pzlib.gzip(codeOrBinaryData);</span>
<span class="cstat-no" title="statement not covered" >        yield _promise.pfs.writeFile(target + '.info', JSON.stringify({ mimeType, dependentFiles }), 'utf8');</span>
      } else {
<span class="cstat-no" title="statement not covered" >        buf = yield _promise.pzlib.gzip(new Buffer(JSON.stringify({ code: codeOrBinaryData, mimeType, dependentFiles })));</span>
      }
&nbsp;
...
&nbsp;
   ...
   */
  getOrFetch<span class="fstat-no" title="function not covered" >(filePath, fetcher) {</span>
<span class="cstat-no" title="statement not covered" >    var _this3 = this;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return _asyncToGenerator(<span class="fstat-no" title="function not covered" >function* () {</span></span>
<span class="cstat-no" title="statement not covered" >      let cacheResult = yield _this3.get(filePath);</span>
<span class="cstat-no" title="statement not covered" >      let anyDependenciesChanged = yield _this3.haveAnyDependentFilesChanged(cacheResult);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if ((cacheResult.code || cacheResult.binaryData) &amp;&amp; !anyDependenciesChanged) {</span>
<span class="cstat-no" title="statement not covered" >        return cacheResult;</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      let result = (yield fetcher(filePath, cacheResult.hashInfo)) || { hashInfo: cacheResult.hashInfo };</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (result.mimeType &amp;&amp; !cacheResult.hashInfo.isInNodeModules) {</span>
<span class="cstat-no" title="statement not covered" >        d(`Cache miss: saving out info for ${filePath}`);</span>
<span class="cstat-no" title="statement not covered" >        yield _this3.save(cacheResult.hashInfo, result.code || result.binaryData, result.mimeType, result.dependentFiles);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        const map = result.sourceMaps;</span>
<span class="cstat-no" title="statement not covered" >        if (map) {</span>
<span class="cstat-no" title="statement not covered" >          d(`source map for ${filePath} found, saving it to ${_this3.getSourceMapPath()}`);</span>
<span class="cstat-no" title="statement not covered" >          yield _this3.saveSourceMap(cacheResult.hashInfo, filePath, map);</span>
        }
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      result.hashInfo = cacheResult.hashInfo;</span>
...
&nbsp;
   ...
   */
  haveAnyDependentFilesChanged<span class="fstat-no" title="function not covered" >(cacheResult) {</span>
<span class="cstat-no" title="statement not covered" >    var _this4 = this;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return _asyncToGenerator(<span class="fstat-no" title="function not covered" >function* () {</span></span>
<span class="cstat-no" title="statement not covered" >      if (!cacheResult.code || !cacheResult.dependentFiles.length) <span class="cstat-no" title="statement not covered" >return false;</span></span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      for (let dependentFile of cacheResult.dependentFiles) {</span>
<span class="cstat-no" title="statement not covered" >        let hasFileChanged = yield _this4.fileChangeCache.hasFileChanged(dependentFile);</span>
<span class="cstat-no" title="statement not covered" >        if (hasFileChanged) {</span>
<span class="cstat-no" title="statement not covered" >          return true;</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        let dependentFileCacheResult = yield _this4.get(dependentFile);</span>
<span class="cstat-no" title="statement not covered" >        if (dependentFileCacheResult.dependentFiles &amp;&amp; dependentFileCacheResult.dependentFiles.length) {</span>
<span class="cstat-no" title="statement not covered" >          let anySubdependentFilesChanged = yield _this4.haveAnyDependentFilesChanged(dependentFileCacheResult);</span>
<span class="cstat-no" title="statement not covered" >          if (anySubdependentFilesChanged) <span class="cstat-no" title="statement not covered" >return true;</span></span>
        }
      }
&nbsp;
...
&nbsp;
  getSync<span class="fstat-no" title="function not covered" >(filePath) {</span>
<span class="cstat-no" title="statement not covered" >    d(`Fetching ${filePath} from cache`);</span>
<span class="cstat-no" title="statement not covered" >    let hashInfo = this.fileChangeCache.getHashForPathSync(_path2.default.resolve(filePath));</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    let code = null;</span>
<span class="cstat-no" title="statement not covered" >    let mimeType = null;</span>
<span class="cstat-no" title="statement not covered" >    let binaryData = null;</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    try {</span>
<span class="cstat-no" title="statement not covered" >      let cacheFile = _path2.default.join(this.getCachePath(), hashInfo.hash);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      let result = null;</span>
<span class="cstat-no" title="statement not covered" >      if (hashInfo.isFileBinary) {</span>
<span class="cstat-no" title="statement not covered" >        d("File is binary, reading out info");</span>
<span class="cstat-no" title="statement not covered" >        let info = JSON.parse(_fs2.default.readFileSync(cacheFile + '.info'));</span>
<span class="cstat-no" title="statement not covered" >        mimeType = info.mimeType;</span>
<span class="cstat-no" title="statement not covered" >        dependentFiles = info.dependentFiles;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        binaryData = hashInfo.binaryData;</span>
<span class="cstat-no" title="statement not covered" >        if (!binaryData) {</span>
<span class="cstat-no" title="statement not covered" >          binaryData = _fs2.default.readFileSync(cacheFile);</span>
<span class="cstat-no" title="statement not covered" >          binaryData = _zlib2.default.gunzipSync(binaryData);</span>
        }
      } else {
<span class="cstat-no" title="statement not covered" >        let buf = _fs2.default.readFileSync(cacheFile);</span>
<span class="cstat-no" title="statement not covered" >        let str = _zlib2.default.gunzipSync(buf).toString('utf8');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        result = JSON.parse(str);</span>
<span class="cstat-no" title="statement not covered" >        code = result.code;</span>
<span class="cstat-no" title="statement not covered" >        mimeType = result.mimeType;</span>
...
<span class="cstat-no" title="statement not covered" >      d(`Failed to read cache for ${filePath}`);</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    return { hashInfo, code, mimeType, binaryData, dependentFiles };</span>
  }
&nbsp;
  saveSync<span class="fstat-no" title="function not covered" >(hashInfo, codeOrBinaryData, mimeType, dependentFiles) {</span>
<span class="cstat-no" title="statement not covered" >    let buf = null;</span>
<span class="cstat-no" title="statement not covered" >    let target = _path2.default.join(this.getCachePath(), hashInfo.hash);</span>
<span class="cstat-no" title="statement not covered" >    d(`Saving to ${target}`);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (hashInfo.isFileBinary) {</span>
<span class="cstat-no" title="statement not covered" >      buf = _zlib2.default.gzipSync(codeOrBinaryData);</span>
<span class="cstat-no" title="statement not covered" >      _fs2.default.writeFileSync(target + '.info', JSON.stringify({ mimeType, dependentFiles }), 'utf8');</span>
    } else {
<span class="cstat-no" title="statement not covered" >      buf = _zlib2.default.gzipSync(new Buffer(JSON.stringify({ code: codeOrBinaryData, mimeType, dependentFiles })));</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    _fs2.default.writeFileSync(target, buf);</span>
  }
&nbsp;
  getOrFetchSync<span class="fstat-no" title="function not covered" >(filePath, fetcher) {</span>
<span class="cstat-no" title="statement not covered" >    let cacheResult = this.getSync(filePath);</span>
<span class="cstat-no" title="statement not covered" >    if (cacheResult.code || cacheResult.binaryData) <span class="cstat-no" title="statement not covered" >return cacheResult;</span></span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    let result = fetcher(filePath, cacheResult.hashInfo) || { hashInfo: cacheResult.hashInfo };</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (result.mimeType &amp;&amp; !cacheResult.hashInfo.isInNodeModules) {</span>
<span class="cstat-no" title="statement not covered" >      d(`Cache miss: saving out info for ${filePath}`);</span>
<span class="cstat-no" title="statement not covered" >      this.saveSync(cacheResult.hashInfo, result.code || result.binaryData, result.mimeType, result.dependentFiles);</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    const map = result.sourceMaps;</span>
<span class="cstat-no" title="statement not covered" >    if (map) {</span>
<span class="cstat-no" title="statement not covered" >      d(`source map for ${filePath} found, saving it to ${this.getSourceMapPath()}`);</span>
<span class="cstat-no" title="statement not covered" >      this.saveSourceMapSync(cacheResult.hashInfo, filePath, map);</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    result.hashInfo = cacheResult.hashInfo;</span>
<span class="cstat-no" title="statement not covered" >    return result;</span>
...
&nbsp;
  buildSourceMapTarget<span class="fstat-no" title="function not covered" >(hashInfo, filePath) {</span>
<span class="cstat-no" title="statement not covered" >    const fileName = _path2.default.basename(filePath);</span>
<span class="cstat-no" title="statement not covered" >    const mapFileName = fileName.replace(_path2.default.extname(fileName), '.js.map');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    const target = _path2.default.join(this.getSourceMapPath(), mapFileName);</span>
<span class="cstat-no" title="statement not covered" >    d(`Sourcemap target is: ${target}`);</span>
&nbsp;
...
&nbsp;
   ...
   */
  saveSourceMap<span class="fstat-no" title="function not covered" >(hashInfo, filePath, sourceMap) {</span>
<span class="cstat-no" title="statement not covered" >    var _this5 = this;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    return _asyncToGenerator(<span class="fstat-no" title="function not covered" >function* () {</span></span>
<span class="cstat-no" title="statement not covered" >      const target = _this5.buildSourceMapTarget(hashInfo, filePath);</span>
<span class="cstat-no" title="statement not covered" >      yield _promise.pfs.writeFile(target, sourceMap, 'utf-8');</span>
    })();
  }
&nbsp;
  saveSourceMapSync<span class="fstat-no" title="function not covered" >(hashInfo, filePath, sourceMap) {</span>
<span class="cstat-no" title="statement not covered" >    const target = this.buildSourceMapTarget(hashInfo, filePath);</span>
<span class="cstat-no" title="statement not covered" >    _fs2.default.writeFileSync(target, sourceMap, 'utf-8');</span>
  }
&nbsp;
  /**
   * @private
   */
  getCachePath<span class="fstat-no" title="function not covered" >() {</span>
    // NB: This is an evil hack so that createFromCompiler can stomp it
    // at will
<span class="cstat-no" title="statement not covered" >    return this.cachePath;</span>
  }
&nbsp;
  /**
   * @private
   */
  getSourceMapPath<span class="fstat-no" title="function not covered" >() {</span>
<span class="cstat-no" title="statement not covered" >    return this.sourceMapPath;</span>
  }
&nbsp;
  /**
